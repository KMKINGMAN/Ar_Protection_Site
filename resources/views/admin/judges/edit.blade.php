<div class="modal-dialog">
    <div class="modal-content model-height">
        ...
        <div class="modal-header">
            <h5 class="modal-title text-dark" id="exampleModalLabel">تعديل القاضى: {{ $judge->username }}</h5>
        </div>
        <form action="{{ route('judges.store') }}" method="POST" enctype="multipart/form-data" id="edit_judge_form">
            @csrf
            @method('PUT')
            <div class="form-group w-100 pr-2 pl-2 m-0">
                ...
                <input id="first_name"
                       ...
                       type="text"
                       value="{{ $judge->first_name }}" required>
            </div>
            ...
            <div class="form-group w-100 pr-2 pl-2 m-0">
                ...
                <input id="last_name"
                       ...
                       type="text"
                       value="{{ $judge->last_name }}" required>
            </div>
            ...
            <div class="form-group w-100 pr-2 pl-2 m-0">
                ...
                <input id="image"
                       ...
                       name="image"
                       type="file">
                <img src="{{ renderImage($judge->image) }}" alt="{{ $judge->username }}" width="100" height="100">
            </div>
        ...
        @else
            <div class="modal-header">
                <h5 class="modal-title text-dark" id="exampleModalLabel">لم يتم العثور على القاضى!</h5>
            </div>
        @endif
...
<script>
    ...
    $('#edit_judge_form').submit(function(e) {
        e.preventDefault();
        let formData = new FormData(this);

        $.ajax({
            type:'POST',
            url: "{{ route('judges.update', $judge->id) }}",
            data: formData,
            contentType: false,
            processData: false,
            success: (response) => {
